module student_t_module
implicit none
use types_mod only KIND_FLOAT
contains
...
! ---------------------------------------------------------
! Returns the value at which the cumulative value of the Student-t distribution with
! nu degrees of freedom is obtained
!  Ej. student_quantile(0.2,10) = 0.879
! ---------------------------------------------------------
	function student_quantile(quantile, nu)
	real(kind=4) :: quantile, nu, student_quantile, beta_log
	integer :: ifault
	real(kind=4) :: x
	
		beta_log = alngam ( nu/2.d0, ifault ) + alngam ( nu/2.d0, ifault ) - alngam ( 1.d0*nu, ifault )
	
		x = xinbta (nu/2.d0, nu/2.d0, beta_log*1.d0, 1.d0*quantile, ifault )
		
		student_quantile = abs((x-0.5) * sqrt(-nu/(x*(x-1.0))))
		
	end function student_quantile
end module student_t_module